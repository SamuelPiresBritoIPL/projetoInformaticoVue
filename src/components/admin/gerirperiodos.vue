  ...
  <div class="container-fluid">
    ...
    <div v-if="this.counterStore.aberturasByCourse.nome != null" class="card text-center">
      ...
      <div class="card-body">
        ...
        <div v-if="navTabConfirmacaoUCs == true">
          ...
          <div v-if="formConfirmacao == false">
            <div v-if="this.counterStore.aberturaConfirmacaoTodos.dataEncerar > new Date().getTime().toLocaleString() || this.counterStore.aberturaConfirmacao1.dataEncerar > new Date().getTime().toLocaleString() || this.counterStore.aberturaConfirmacao2.dataEncerar > new Date().getTime().toLocaleString() || this.counterStore.aberturaConfirmacao3.dataEncerar > new Date().getTime().toLocaleString()" class="row text-start">
              <div v-if="this.counterStore.aberturaConfirmacaoTodos.length != 0" class="row">
                ...
                <div>
                  <button type="button" class="btn btn-primary me-1" @click="openEditarConfirmacao(this.counterStore.aberturaConfirmacaoTodos)"><i class="align-baseline bi bi-pencil-square"></i> Editar</button>
                  <button type="button" class="btn btn-danger" @click="deleteAbertura(this.counterStore.aberturaConfirmacaoTodos.id)"><i class="align-baseline bi bi-calendar2-check"></i> Encerrar</button> 
                </div>
              </div>
              <div v-else>
                <div v-if="this.counterStore.aberturaConfirmacao123.length != 0" >
                  <div v-for="anoAbertura in this.counterStore.aberturaConfirmacao123" :key="anoAbertura" class="row">
                    ...
                    <div>
                      <button type="button" class="btn btn-primary me-1" @click="openEditarConfirmacao(anoAbertura)"><i class="align-baseline bi bi-pencil-square"></i> Editar</button>
                      <button type="button" class="btn btn-danger" @click="deleteAbertura(anoAbertura.id)"><i class="align-baseline bi bi-calendar2-check"></i> Encerrar</button> 
                    </div> 
                  </div>
                </div>
              </div>
          ...
          <div v-if="formConfirmacao == true">
            ...
            <form>
              ...
              <button v-if="editarConfirmacao == true" type="button" class="btn btn-primary me-1" 
                @click="updateAbertura(0, dataAbertura, dataEncerrar)"
                ><i class="align-baseline bi bi-check-lg"></i> Confirmar Edição</button>
              <button v-if="iniciarConfirmacao == true" type="button" class="btn btn-primary me-1" 
        ...
        <div v-else>
          <h6 class="card-title mb-4">Período de Inscrição nos Turnos - {{ "["+this.counterStore.aberturasByCourse.codigo+"] "+this.counterStore.aberturasByCourse.nome }}</h6>
          <div v-if="formInscricao == false">
            ...
            <div v-if="this.counterStore.aberturaInscricaoTodos.dataEncerar > new Date().getTime().toLocaleString() || this.counterStore.aberturaInscricao1.dataEncerar > new Date().getTime().toLocaleString() || this.counterStore.aberturaInscricao2.dataEncerar > new Date().getTime().toLocaleString() || this.counterStore.aberturaInscricao3.dataEncerar > new Date().getTime().toLocaleString()" class="row text-start">
              <div v-if="this.counterStore.aberturaInscricaoTodos.length != 0" class="row">
                ...
                <div>
                  <button type="button" class="btn btn-primary me-1" @click="openEditarInscricao(this.counterStore.aberturaInscricaoTodos)"><i class="align-baseline bi bi-pencil-square"></i>  Editar</button>
                  <button type="button" class="btn btn-danger" @click="deleteAbertura(this.counterStore.aberturaInscricaoTodos.id)"><i class="align-baseline bi bi-calendar2-check"></i> Encerrar</button> 
                </div>
              </div>
              <div v-else>
                <div v-if="this.counterStore.aberturaInscricao123.length != 0" >
                  <div v-for="anoAbertura in this.counterStore.aberturaInscricao123" :key="anoAbertura" class="row">
                    ...
                    <label class="col-sm-10 col-form-label">{{ anoAbertura.dataEncerar.replace(':00.000000Z', '').replace('T', ' ') }}</label>
                    <div>
                      <button type="button" class="btn btn-primary me-1" @click="openEditarInscricao(anoAbertura)"><i class="align-baseline bi bi-pencil-square"></i> Editar</button>
                      <button type="button" class="btn btn-danger" @click="deleteAbertura(anoAbertura.id)"><i class="align-baseline bi bi-calendar2-check"></i> Encerrar</button> 
                    </div> 
                  </div>
                </div>
          ...
          <div v-if="formInscricao == true">
            <form>
              ...
              <button v-if="editarInscricao == true" type="button" class="btn btn-primary me-1"
              @click="updateAbertura(selectedYear, dataAbertura, dataEncerrar)"><i class="align-baseline bi bi-check-lg"></i> Confirmar Edição</button>
              <button v-if="iniciarInscricao == true" type="button" class="btn btn-primary me-1"
