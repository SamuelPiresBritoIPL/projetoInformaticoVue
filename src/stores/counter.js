  ...
  actions: {
    ...
    async getCourses(tipo = 0) {
      try {
        ...
        this.coursesToVSelectComTodos.push({ label: "Todos", code: 0 });
        this.courses.forEach((curso) => {
          this.coursesToVSelect.push({
            label: "[" + curso.codigo + "] " + curso.nome,
            code: curso.id,
          });
          this.coursesToVSelectComTodos.push({
            label: "[" + curso.codigo + "] " + curso.nome,
            code: curso.id,
          });
        });
    ...
    async login(credentials, tipoLogin) {
      ...
      try {
        ...
        if (
          ...
          response.data.tipo == 1 &&
          response.data.isCoordenador == 1 &&
          tipoLogin == 2
        ) {
          ...
          sessionStorage.setItem(
            "isCoordenadorPrincial",
            response.data.isCoordenadorPrincipal
          );
        ...
        if (
          response.data.access_token &&
          response.data.tipo == 3 &&
          response.data.isProfessor == 1 &&
          tipoLogin == 1
        ) {
